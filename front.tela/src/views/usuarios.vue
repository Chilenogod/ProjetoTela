<template>
  <div>
    ...
    <v-data-table :headers="headers" :items="usuarios" :items-per-page="5" dense @click:row="atualizaUsuario">
      ...
      <template v-slot:[`item.action`]="{ item }">
        <v-icon small class="mr-2" @click.stop="atualizaUsuario(item)" color="primary" title="Alterar Usuário">mdi-pencil</v-icon>
        <v-icon small class="mr-2" @click.stop="apagaUsuario(item)" color="error" title="Excluir Usuário">mdi-delete</v-icon>
      </template>
    </v-data-table>
